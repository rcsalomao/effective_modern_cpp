...
void main() {
    ...
    // 'T' de uma expressão template (item 1). O especificador do tipo faz o
    ...
    // com a ressalva de 1 caso que se trata de uma exceção. seguindo a lógica
    ...
    auto ix = rx;        // caso 3
    auto&& uref1 = x;   // caso 2 (int&). 'referencing collapsing': (E)&& -> E&.
    auto&& uref2 = cx;  // caso 2 (const int&). 'ref collapsing': (E)&& -> E&.
    auto&& uref3 = rx;  // caso 2 (const int&). 'ref collapsing': (E&)&& -> E&.
    auto&& uref4 = 47;  // caso 2 (int&&). 'ref collapsing': (E&&)&& -> E&&.
